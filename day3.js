...
const markMap = (coordinate, marker) => {
  const stringified = coordinate.toString()
  const marked = wiresMap.get(stringified)
  if (!marked) {
    wiresMap.set(stringified, marker)
  } else if (marked !== marker) {
    wiresMap.set(stringified, 'X')
    intersections.push(coordinate)
  } // otherwise it's the same line, which we can ignore

  return coordinate
}

const move = ([x, y], stepNum, marker, op) => {
  let step
  for (let i = 1; i <= stepNum; i++) {
    step = markMap(op([x, y], i), marker)
  }
  return step
}
...
const processInstruction = (instruction, pointer, marker) => {
  // console.log('starting at:', pointer)
  const [_, direction, stepNum] = instruction.match(/([U|D|L|R])(\d*)/)
  switch (direction) {
    case 'U':
      pointer = move(pointer, stepNum, marker, up)
      break
    case 'D':
      pointer = move(pointer, stepNum, marker, down)
      break
    case 'L':
      pointer = move(pointer, stepNum, marker, left)
      break
    case 'R':
      pointer = move(pointer, stepNum, marker, right)
      break
    default:
      break
  }
  // console.log('ending at:', pointer)
  return pointer
}
...
const processInstructions = (input, marker) => {
  ...
  input.trim().split(',').forEach(instruction => {
    pointer = processInstruction(instruction, pointer, marker)
  })
}
...
const distances = intersections.map(([x, y]) => Math.abs(x) + Math.abs(y)).sort((a, b) => a - b)
