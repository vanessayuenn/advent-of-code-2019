...
const markMap = (coordinate, marker, stepsSoFar, steps) => {
  ...
  } // else: it's the same line, which we can ignore
  return [coordinate, stepsTravelled]
}

const move = (pointer, stepNum, marker, op, stepsSoFar) => {
  let coordinate = pointer, newsteps = stepsSoFar
  for (let i = 1; i <= stepNum; i++) {
    [coordinate, newsteps] = markMap(op(pointer, i), marker, stepsSoFar, i)
    // console.log(`${marker} took ${newsteps} steps to arrive at ${coordinate}`)
  }
  return [coordinate, newsteps]
}

const up = ([x, y], i) => [x, y + i]
const down = ([x, y], i) => [x, y - i]
const left = ([x, y], i) => [x - i, y]
const right = ([x, y], i) => [x + i, y]

const processInstruction = (instruction, pointer, marker, stepsSoFar) => {
  const [_, direction, stepNum] = instruction.match(/([U|D|L|R])(\d*)/)
  switch (direction) {
    case 'U':
      [pointer, stepsSoFar] = move(pointer, stepNum, marker, up, stepsSoFar)
      break
    case 'D':
      [pointer, stepsSoFar] = move(pointer, stepNum, marker, down, stepsSoFar)
      break
    case 'L':
      [pointer, stepsSoFar] = move(pointer, stepNum, marker, left, stepsSoFar)
      break
    case 'R':
      [pointer, stepsSoFar] = move(pointer, stepNum, marker, right, stepsSoFar)
      break
    default:
      break
  }
  return [pointer, stepsSoFar]
}

const processInstructions = (input, marker) => {
  let pointer = [0, 0]
  let stepsSoFar = 0
  input.trim().split(',').forEach(instruction => {
    [pointer, stepsSoFar] = processInstruction(instruction, pointer, marker, stepsSoFar)
  })
}

processInstructions(input[0], 'L1')
processInstructions(input[1], 'L2')

// console.log(intersections, intersectionsWithSteps)

// sort intersections ([x,y]) by distance which is x+y
